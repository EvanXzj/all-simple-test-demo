...
class HandleDB {
    ...
    constructor(options) {
        this.databaseFile = options && options.databaseFile || `test.db`;    // 数据库文件(文件路径+文件名)
    ...
    createTable(sentence) {
        ...
        return new Promise((resolve, reject) => {
            _self.db.exec(sentence, function(err) {
                if (err) {
                    if (err.message === 'SQLITE_ERROR: table baby already exists') {
                        resolve('表已存在，不需要重新创建该表')
                    }
                    reject(err)
                };
                resolve(`表创建成功`);
